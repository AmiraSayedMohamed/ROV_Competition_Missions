...
KNOWN_DISTANCE = 5.0  # Known distance in cm
KNOWN_LENGTH = 30.0  # Reference object length in cm
reference_length = KNOWN_LENGTH
ratio = 1.0
...
while cap.isOpened():
    ...
    if len(points) == 2:
        ...
        scaling_factor = KNOWN_DISTANCE / avg_depth
        cm_distance = pixel_distance * scaling_factor
    ...
    if fixed_measurement is not None:
        ...
        if ratio != 1.0:
            corrected_distance = fixed_measurement * ratio
    ...
    if key == ord('q'):
        if len(points) == 2:
            ...
            scaling_factor = KNOWN_DISTANCE / avg_depth
            cm_distance = pixel_distance * scaling_factor
            ratio = reference_length / cm_distance
